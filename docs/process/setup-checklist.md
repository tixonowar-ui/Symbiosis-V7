...
Владелец — `@tixonowar-ui`, уже проставлен.

**Осталось решить одно.** В файле есть плейсхолдер `@codex-agent` на `/src/` и
`/tests/`. Он **не сработает как есть**: CODEOWNERS принимает только
пользователей и команды, а GitHub App владельцем кода быть не может.

Варианты:

- [ ] Если Codex открывает PR под бот-аккаунтом — заменить `@codex-agent` на
      его реальный логин
- [ ] Если под вашим аккаунтом — **удалить обе строки** с `@codex-agent`,
      правило `*` и так покрывает
- [ ] Если появится организация — завести команду и указать её

Проверка: откройте файл на GitHub. Невалидный логин подсвечивается
предупреждением прямо в интерфейсе. Пока предупреждение есть, правило
`Require review from Code Owners` работать не будет.
